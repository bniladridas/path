...
test.describe("Navigation", () => {
  ...
  test("should have working navigation links", async ({ page, baseURL }) => {
    ...
    try {
      ...
      const navLinks = await page.$$eval(SELECTORS.NAV_LINKS, (elements) =>
        elements
          .filter(
            (el) =>
              el.href &&
      ...
      const mainNavItems = [
        {
          selector: '[data-testid="nav-terms"]',
          url: URLS.TERMS,
          isExternal: false,
        },
        {
          selector: '[data-testid="nav-privacy"]',
          url: URLS.PRIVACY,
          isExternal: false,
        },
      ];
      ...
      for (const [index, navItem] of allNavItems.entries()) {
        ...
        console.log(
          `\nTesting navigation [${index + 1}/${testResults.total}]:`,
          navItem.selector,
        );
        ...
        try {
          ...
          if (isExternal) {
            ...
            if (elementInfo.href === expectedUrl) {
              ...
              testResults.passed++;
              console.log(
                `\x1b[32m[PASS]\x1b[0m External link has correct href: ${elementInfo.href}`,
              );
            } else {
              ...
              testResults.failed++;
              console.log(
                `\x1b[31m[FAIL]\x1b[0m External link href mismatch: expected ${expectedUrl}, got ${elementInfo.href}`,
              );
            }
          } else {
            ...
            if (isSuccess) {
              ...
              testResults.passed++;
              console.log(
                `\x1b[32m[PASS]\x1b[0m Navigation successful to: ${currentUrl.toString()}`,
              );
            } else {
              result.status = "failed";
              ...
              testResults.failed++;
              console.log(
                `\x1b[31m[FAIL]\x1b[0m Navigation failed or stayed on the same page`,
              );
            }
          }
        } catch (error) {
          ...
          testResults.failed++;
          console.error(
            `\x1b[31m[ERROR]\x1b[0m Error testing navigation [${navItem.selector}]:`,
            error.message,
          );
      ...
      if (testResults.passed === 0) {
        console.warn(
          "No navigation links were tested successfully. Taking a screenshot...",
        );
